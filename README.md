...
> - **Docker/Compose 拓扑**：默认 Compose 拓扑已拆分为 `api` / `worker-panowan` / `model-setup` 三类运行角色。
...
模型准备由独立的 Model Setup 角色完成：

```bash
make setup-models
```

这会运行一次性 `model-setup` 容器下载与校验模型资产，使服务启动与 asset preparation 明确分离。
