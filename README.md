...
>
- **Docker/Compose 拓扑**：默认 Compose 拓扑已拆分为 `api` / `worker-panowan` 两类运行角色；宿主机侧初始化通过 `make setup` 或 `make setup-worktree` 完成。
...
| Host setup (`make setup` / `make setup-worktree`) | 主仓库下载并校验模型权重；worktree 复用共享数据，并校验 backend vendor tree 与当前 checkout 的本地 runtime 前置条件 | 否 | 是 |
...
- **环境诊断**：`make verify` 运行测试并检测 GPU、Docker、模型文件等配置。
...
> **WSL2 用户**：请确保已安装 NVIDIA Container Toolkit。若有问题，运行 `make verify` 自动检测并引导修复。
...
如果你是在额外 Git worktree 中开发，仅将第一步替换为 `make setup-worktree`。
...
cd panowan-web-ui
make setup        # 完成当前 checkout 的标准初始化
```

如果你是在新的 Git worktree 中首次进入，请改用：

```bash
make setup-worktree
```

这会先链接共享 `data/models`，再完成当前 worktree 的 Python 环境准备、backend vendor tree 校验、本地 runtime 前置条件校验与诊断步骤；它不会在 worktree 内重新下载 HuggingFace 模型。如需共享 `data/runtime`，可显式运行：

```bash
make setup-worktree WITH_RUNTIME=1
```
...
`make setup` 会下载并校验主仓库所需模型；`make setup-worktree` 会复用 worktree 共享数据，但仍会校验 backend vendor tree 与当前 checkout 的本地 runtime 前置条件。只有在做定向维护时，才需要直接调用脚本或 CLI：

```bash
make setup            # 主仓库重新安装 / 校验 backend 资产
make setup-worktree   # worktree 重新链接共享数据并重跑本地校验
bash scripts/doctor.sh
bash scripts/data-sync.sh status
```

如需绕过工作流入口做底层维护，再直接调用 `.venv/Scripts/python.exe -m app.backends install`。
...
开发 override 通过 `docker-compose-dev.yml` 注入，仅作为开发便利层，不改变默认产品拓扑。`make dev` 等价于 `make up DEV=1`。
...
`make verify` 是标准本地验证入口；如需只做定向环境诊断，可直接运行 `bash scripts/doctor.sh`。
...
```bash
make setup          # main checkout onboarding
make setup-worktree # worktree onboarding
...
make dev            # start split topology with dev overrides
...
- 确保 GPU 被 Docker 正确识别：`bash scripts/doctor.sh`。
...
诊断脚本会交互式引导安装 NVIDIA Container Toolkit。详细步骤见 [ENVIRONMENT.md](ENVIRONMENT.md)。
